...
'''Path for excel file final generated'''   
def change_excel_final_path(path):
    if(os.path.exists(os.path.join(path,"excel_final")) == False):
        os.mkdir(os.path.join(path,"excel_final"))
    dict_configuration_path["excel_final_path"] = os.path.join(path, "excel_final")
    utility.saveJsonFile(os.path.join(os.path.realpath(''), "configuration", "pathConfiguration.json"), dict_configuration_path)
    return True
...
def control_paths():
    ...
    if(os.path.exists(hierarchy_path) == False):  
        os.mkdir(os.path.join(hierarchy_path))

    if(os.path.exists(excel_files_path) == False):
        os.mkdir(os.path.join(excel_files_path))

    if(os.path.exists(excel_final_path) == False):
        os.mkdir(os.path.join(excel_final_path))

    if(os.path.exists(json_templates_path) == False):
        os.mkdir(os.path.join(json_templates_path))           
