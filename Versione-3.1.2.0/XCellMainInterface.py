...
class XCellMainInterface(QtWidgets.QMainWindow):
    def __init__(self, obj, parent=None):
        ...
        '''Change measure set'''
        change_measures_set_act = QtWidgets.QAction(QtGui.QIcon(os.path.join(os.path.realpath(''), "image", "icon","checkicon.png")), 'Change measures set', self)
        change_measures_set_act.setShortcut('Ctrl+Q')
        change_measures_set_act.setStatusTip('Change measures set')

        '''Add Trend ID'''
        add_trend_id_act = QtWidgets.QAction(QtGui.QIcon(os.path.join(os.path.realpath(''), "image", "icon","addicon.png")), 'Add Trend ID', self)
        add_trend_id_act.setShortcut('Ctrl+Q')
        add_trend_id_act.setStatusTip('Add Trend ID')
        
        '''Act toolbar'''
        toolbar.addAction(back_act)
        toolbar.addAction(change_measures_set_act)
        toolbar.addAction(add_trend_id_act)

        '''Toolbar triggered'''
        back_act.triggered.connect(self.returnFirstInterface)
        change_measures_set_act.triggered.connect(self.openChangeMeasuresSetInterface)
        add_trend_id_act.triggered.connect(self.addTrendId)
    ...
    def openChangeMeasuresSetInterface(self):
        dialog = ToolbarInterface.ChangeMeasuresSetInterface(self)
        dialog.show()

    def addTrendId(self):
        dialog = ToolbarInterface.AddTrendIdInterface(self)
        dialog.show()